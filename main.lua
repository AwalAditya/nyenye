...
if not uiLoadSuccess or not WindUI then
    warn("❌ Failed to load WindUI: " .. tostring(uiLoadError))
...
-- Tunggu character spawn dengan safety check
...
local success, err = pcall(function()
    local charactersFolder = workspace:WaitForChild("Characters", 30)
    if not charactersFolder then
        error("Characters folder not found in workspace")
    end
    
    _G.Characters = charactersFolder:WaitForChild(LocalPlayer.Name, 30)
    if not _G.Characters then
        error("Player character not found in Characters folder")
    end
    
    _G.HRP = _G.Characters:WaitForChild("HumanoidRootPart", 10)
    _G.Overhead = _G.HRP:WaitForChild("Overhead", 10)
    _G.Header = _G.Overhead:WaitForChild("Content", 10):WaitForChild("Header", 10)
    _G.LevelLabel = _G.Overhead:WaitForChild("LevelContainer", 10):WaitForChild("Label", 10)
    
    _G.XPBar = LocalPlayer:WaitForChild("PlayerGui", 10):WaitForChild("XP", 10)
    _G.XPLevel = _G.XPBar:WaitForChild("Frame", 10):WaitForChild("LevelCount", 10)
    _G.Title = _G.Overhead:WaitForChild("TitleContainer", 10):WaitForChild("Label", 10)
    _G.TitleEnabled = _G.Overhead:WaitForChild("TitleContainer", 10)
end)

if not success then
    warn("⚠️ Warning: Some character elements couldn't load: " .. tostring(err))
    warn("⚠️ Script will continue, but some features may not work properly")
end
...
task.spawn(function()
    pcall(function()
        ...
        if _G.TitleEnabled and _G.Title then
            ...
            while task.wait(1.5) do
                if not _G.Title or not _G.Title.Parent then break end
...
local RodIdleAnim, RodShakeAnim, RodIdleTrack, RodShakeTrack

pcall(function()
    local ModulesFolder = ReplicatedStorage:WaitForChild("Modules", 10)
    local AnimationsFolder = ModulesFolder:WaitForChild("Animations", 10)
    
    RodIdleAnim = AnimationsFolder:WaitForChild("ReelingIdle", 10)
    RodShakeAnim = AnimationsFolder:WaitForChild("RodThrow", 10)
    ...
    if humanoid then
        ...
        if RodShakeAnim then
            RodShakeTrack = animator:LoadAnimation(RodShakeAnim)
        end
        if RodIdleAnim then
            RodIdleTrack = animator:LoadAnimation(RodIdleAnim)
        end
    end
end)
...
You can buy a Premium for the custom Channel you want.
]]
})
...
function _G.StartAutoSecret()
    task.spawn(function()
        ...
        task.wait(5)
        LocalPlayer:Kick("Ciee Berharap Banget Secret tuh HAHAHA. (" .. randomId .. " | " .. randomWeight .. "kg)")
    end)
end
...
-------------------------------------------
----- =======[ X5 SPEED FISHING TAB ]
-------------------------------------------

-- Modules untuk X5 Speed (menggunakan modules yang sudah di-load)
local X5Modules = {}

-- Tunggu sebentar untuk memastikan main modules sudah ready
task.wait(1)

local x5Success, x5ErrorMessage = pcall(function()
    -- Reuse existing modules instead of loading new ones to prevent conflict
    if _G.FishingController then
        X5Modules.FishingController = _G.FishingController
    end
        ...
        if netPath then
            ...
            if netPath then
                ...
                if netPath then
                    ...
                    if netPath then
                        X5Modules.EquipToolEvent = netPath:FindFirstChild("RE/EquipToolFromHotbar")
                        X5Modules.ChargeRodFunc = netPath:FindFirstChild("RF/ChargeFishingRod")
                        X5Modules.StartMinigameFunc = netPath:FindFirstChild("RF/RequestFishingMinigameStarted")
                        X5Modules.CompleteFishingEvent = netPath:FindFirstChild("RE/FishingCompleted")
                    end
                end
            end
        end
    ...
    if not netSuccess then
        warn("⚠️ X5 Speed: Network modules not found, X5 Speed may not work")
    end
end)

if not x5Success then
    warn("⚠️ X5 Speed Module Loading Error: " .. tostring(x5ErrorMessage))
    warn("⚠️ X5 Speed fishing will be disabled, but other features will work")
end

local x5FeatureState = {
    AutoFish = false,
...
local function equipFishingRodX5()
    if X5Modules.EquipToolEvent then
        pcall(X5Modules.EquipToolEvent.FireServer, X5Modules.EquipToolEvent, 1)
    end
end
...
local function stopAutoFishProcessesX5()
    x5FeatureState.AutoFish = false
    ...
    pcall(function()
        if X5Modules.FishingController and X5Modules.FishingController.RequestClientStopFishing then
            X5Modules.FishingController:RequestClientStopFishing(true)
        end
    end)
end

local function startAutoFishMethod_InstantX5()
    if not (X5Modules.ChargeRodFunc and X5Modules.StartMinigameFunc and X5Modules.CompleteFishingEvent and X5Modules.FishingController) then
        return
    end

    x5FeatureState.AutoFish = true
    ...
    local function worker()
        while x5FeatureState.AutoFish and LocalPlayer do
            local currentResetTarget_Worker = x5FeatureState.Instant_ResetCount or 10
            ...
            local success, err = pcall(function()
                ...
                X5Modules.ChargeRodFunc:InvokeServer(nil, nil, nil, workspace:GetServerTimeNow())
                task.wait(x5FeatureState.Instant_ChargeDelay)
                X5Modules.StartMinigameFunc:InvokeServer(-139, 1, workspace:GetServerTimeNow())
                task.wait(x5FeatureState.Instant_StartDelay)

                if not x5FeatureState.AutoFish or isCurrentlyResetting then return end

                for _ = 1, x5FeatureState.Instant_SpamCount do
                    if not x5FeatureState.AutoFish or isCurrentlyResetting then break end
                    X5Modules.CompleteFishingEvent:FireServer()
                    task.wait(0.05)
                end
                ...
                local connection
                local timeoutThread = task.delay(x5FeatureState.Instant_CatchTimeout, function()
                ...
                if X5Modules.FishingController and X5Modules.FishingController.RequestClientStopFishing then
                    pcall(X5Modules.FishingController.RequestClientStopFishing, X5Modules.FishingController, true)
                end
            ...
            if not success then
                warn("X5 Auto Instant Fish Error: ", err)
                task.wait(1)
            end

            if not x5FeatureState.AutoFish then break end
            task.wait(x5FeatureState.Instant_CycleDelay)
        end
    end

    x5AutoFishThread = task.spawn(function()
        while x5FeatureState.AutoFish do
            local currentResetTarget = x5FeatureState.Instant_ResetCount or 10
            local currentPauseTime = x5FeatureState.Instant_ResetPause or 0.01
            ...
            for i = 1, x5FeatureState.Instant_WorkerCount do
                if not x5FeatureState.AutoFish then break end
            ...
            while x5FeatureState.AutoFish and chargeCount < currentResetTarget do
                task.wait()
            end

            isCurrentlyResetting = true

            if x5FeatureState.AutoFish then
        ...
        stopAutoFishProcessesX5()
    end)

    table.insert(x5FishingTrove, x5AutoFishThread)
end

local function startOrStopAutoFishX5(shouldStart)
    if shouldStart then
        stopAutoFishProcessesX5()
        x5FeatureState.AutoFish = true
        equipFishingRodX5()
        task.wait(0.01)
        startAutoFishMethod_InstantX5()
        NotifySuccess("X5 Speed", "X5 Speed Auto Fishing Started!")
    else
        stopAutoFishProcessesX5()
        NotifyInfo("X5 Speed", "X5 Speed Auto Fishing Stopped!")
    end
end

-- UI untuk X5 Speed Tab
X5SpeedTab:Section({ Title = "X5 Speed Settings", Opened = true })

X5SpeedTab:Paragraph({
    Title = "⚡ X5 Speed Fishing",
    Color = "Yellow",
    Desc = [[
Ultra-fast fishing mode with instant catch!
Adjust settings below for optimal performance.

⚠️ WARNING: 
- Do NOT use X5 Speed together with regular Auto Fish!
- Use ONLY ONE auto fishing method at a time
...
X5SpeedTab:Slider({
    Title = "Delay Recast",
    Desc = "Delay between casts (Default: 1.20)",
    Value = { Min = 0.00, Max = 5.0, Default = x5FeatureState.Instant_StartDelay },
    Precise = 2,
    Step = 0.01,
    Callback = function(v)
        x5FeatureState.Instant_StartDelay = tonumber(v)
    end
})

X5SpeedTab:Slider({
    Title = "Spam Finish Count",
    Desc = "How many times to spam finish (Default: 10)",
    Value = { Min = 5, Max = 50, Default = x5FeatureState.Instant_ResetCount },
    Precise = 0,
    Step = 1,
    Callback = function(v)
        x5FeatureState.Instant_ResetCount = math.floor(tonumber(v) or 10)
    end
})

X5SpeedTab:Slider({
    Title = "Cooldown Recast",
    Desc = "Cooldown between recasts (Default: 0.01)",
    Value = { Min = 0.01, Max = 5, Default = x5FeatureState.Instant_ResetPause },
    Precise = 2,
    Step = 0.01,
    Callback = function(v)
        x5FeatureState.Instant_ResetPause = tonumber(v) or 0.01
    end
})

X5SpeedTab:Section({ Title = "X5 Auto Fishing", Opened = true })

X5SpeedTab:Toggle({
    Title = "Start X5 AutoFish",
    Desc = "Enable ultra-fast auto fishing",
    Value = false,
    Callback = startOrStopAutoFishX5
})

-- No Animation Feature
X5SpeedTab:Space()
X5SpeedTab:Section({ Title = "Animation Control", Opened = true })
...
local function setGameAnimationsEnabled(state)
    ...
    if state then
        ...
        NotifyInfo("X5 Speed", "All game animations disabled!")
    else
        for _, conn in pairs(stopAnimConnections) do
            conn:Disconnect()
        end
        ...
        NotifyInfo("X5 Speed", "Game animations re-enabled!")
    end
end

X5SpeedTab:Toggle({
    Title = "No Animation",
    Desc = "Disable all game animations for better performance",
    Value = false,
    Callback = function(v)
