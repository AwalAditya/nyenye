...
-- Modules untuk X5 Speed
local X5Modules = {}
local function customRequireX5(module)
...
local x5Success, x5ErrorMessage = pcall(function()
    ...
    X5Modules.Replion = customRequireX5(ReplicatedStorage.Packages.Replion)
    X5Modules.ItemUtility = customRequireX5(Shared.ItemUtility)
    X5Modules.FishingController = customRequireX5(Controllers.FishingController)
    
    X5Modules.EquipToolEvent = NetFolder["RE/EquipToolFromHotbar"]
    X5Modules.ChargeRodFunc = NetFolder["RF/ChargeFishingRod"]
    X5Modules.StartMinigameFunc = NetFolder["RF/RequestFishingMinigameStarted"]
    X5Modules.CompleteFishingEvent = NetFolder["RE/FishingCompleted"]
end)

if not x5Success then
    warn("X5 Speed Module Loading Error: " .. tostring(x5ErrorMessage))
end
